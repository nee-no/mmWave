...
Date: 10/10/2019
Title: models/reinforcement/test_method1.py (original3_foo.py)
Code Version: 
Availability: 
''' 
'''
This file defines the test environment (gym environment) used in testing the DQN (dqn.py/dqn.ipynb).
Train file is method1_reward.py
'''
import gym
...
class FooEnv(gym.Env):
    ...
    def _update_state(self, action):
        ...
        self._weights = (np.argsort(self._power_available[self.count, :]))/(self._power_available.shape[1]-1) # to weigh which is action taken has more contribution to reward
        ...
        self._acc_counter += self._weights[int(action)]
